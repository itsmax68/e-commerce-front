...
export const Reviews = ({productId,averageRating}) => {
    ...
    // reviews coming from API can occasionally contain `user: null`
    // so we guard while counting rating distribution.
    reviews?.forEach((review) => {
        const rating = review?.rating;
        if (typeof rating === 'number' && ratingCounts[rating] !== undefined) {
    ...
    const handleAddReview=(data)=>{
        if(!loggedInUser){
            // public users can read reviews, but can't post them
            return;
        }
